...
class case7(unittest.TestCase):
    ...
    Должно создаться три заявления, у каждого заполнен IZM"""
    ...
    def delASP(self, cur, FIO):
        ...
        if f2_id:
            ...
            (select id from F6 where F2_ID in (%s)))""" % f2_id)
            # удалим о ребенке
            cur.execute("select id from F2 where FAMIL=? and IMJA=? and OTCH=?",
                        ('ФамилияРебенка', 'ИмяРебенка', 'ОтчествоРебенка'))
            idListChild = list()
            for id_Child in cur.fetchall():
                idListChild.append(str(id_Child[0]))
            if idListChild:
                f2_id_Child = ','.join(idListChild)
                cur.execute("DELETE F_PENSFAM where f2_id in (%s)" % f2_id_Child)
    ...
    def test_1_loadZaivl(self):
        """Загружает заявление для Пособие Семьям Детям №family_2"""
        # человек
...
lastName=? and firstName=? and middleName=? and requestId='family_2'""", fio)
        # отсоединяюсь от ТИ
        con.commit()
        con.close()
        # соединяюсь с АСП
...
delete EService_Request where f6_id is NULL and f6izm_id is NULL""")
    ...
    def test_2_writeZaivl(self):
        """Регистрирует заявление, проверяет создание 1-х заявление с заполненными F6 и F6_izm."""
        fio = ('Пособие', 'Семьям', 'Детям')
        # захожу в госуслуги
        driver = self.driver
        driver.get(self.base_url + "VisitingService/ViewGosUsl.aspx")
    ...
    -- отметить все заявки района как не загруженную
    update EService_Request set exportDate=NULL, exportFile=NULL where EService_Users_id = @id and
    lastName=? and firstName=? and middleName=? and requestId in ('family_3', 'family_4')""", fio)
        # отсоединяюсь от ТИ
        con.commit()
        con.close()
        driver = self.driver
